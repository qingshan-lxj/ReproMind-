...
public class ChatController {
    ...
    /**
     * 论文复现实验诊断接口（SSE 流式模式）- 自动规划排查步骤并生成复现实验诊断报告
     * 无需用户输入，自动执行论文复现诊断流程
     */
    @PostMapping(value = "/ai_ops", produces = "text/event-stream;charset=UTF-8")
    public SseEmitter aiOps(@RequestBody(required = false) AIOpsRequest request) {
        SseEmitter emitter = new SseEmitter(600000L); // 10分钟超时（复现实验诊断可能较慢）

        executor.execute(() -> {
            try {
                logger.info("收到论文复现实验诊断请求 - 启动多 Agent 协作流程");
                ...
                Optional<OverAllState> overAllStateOptional = aiOpsService.executeAiOpsAnalysis(chatModel, toolCallbacks, request);
                ...
                logger.info("论文复现实验诊断编排完成，开始提取最终报告...");
                ...
                emitter.complete();
                logger.info("论文复现实验诊断多 Agent 编排完成");

            } catch (Exception e) {
                logger.error("论文复现实验诊断多 Agent 协作失败", e);
                try {
                    emitter.send(SseEmitter.event().name("message")
                            .data(SseMessage.error("论文复现实验诊断流程失败: " + e.getMessage()), MediaType.APPLICATION_JSON));
                } catch (IOException ex) {
                    logger.error("发送错误消息失败", ex);
                }
                emitter.completeWithError(e);
